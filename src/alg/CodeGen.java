...
//    }
    ...
    //  expression: expression L_BRACKET expression R_BRACKET   #IndexArr//Indexecao do array
    public Symbol visitIndexArr(alg.IndexArrContext ctx) {
        Symbol t1 = visit(ctx.expression(0));
        Symbol t2 = visit(ctx.expression(1));

        // place in memory _t0 = size * number
        Symbol _t0 = this.temp(ctx, t2.type);
        emit(_t0.name + " = " + t2.type.getWidth() + " * " + t2.name);
